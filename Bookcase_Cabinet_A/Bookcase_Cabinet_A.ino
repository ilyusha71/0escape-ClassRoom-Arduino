...
#define SHOW_TAG_ID
/* Constant for Gadget Tag ID */
const int tagPoker[4] = {133,129,52,0};
const int tagBus[4] = {16,19,44,51};
/* Output */
...
void setup() 
{
    ...
    pinMode(passBus,OUTPUT);
    digitalWrite(passPoker, LOW);
    digitalWrite(passBus, HIGH);
    Serial.println("Bookcase - Cabinet A 2016/09/15 iLYuSha Wakaka KocmocA");
}
...
{ 
  if(escape == -1)
  {
    ...
    else if(mfrc522.uid.uidByte[0] == tagBus[0] && mfrc522.uid.uidByte[1] == tagBus[1] && mfrc522.uid.uidByte[2] == tagBus[2] && mfrc522.uid.uidByte[3] == tagBus[3])
    {
      digitalWrite(passBus, LOW);
      Serial.print("Bus Bingo ");
      ShowTagID();
      Serial.println("");
    }
  }
