...
void setup() {
  ...
  pinMode(msgA2D, OUTPUT);
  digitalWrite(teacherDoor, LOW);
  digitalWrite(secretDoor, LOW);
  digitalWrite(msgA2D, HIGH);
  Serial.println("Ghost Class Room D 2016/08/06 iLYuSha Wakaka KocmocA");
}
...
//  }
  ...
  // A椅子 放 D位置 觸發 -> 語音播放 -> 進入A位置語音10秒延時
  ...
  {
    digitalWrite(msgA2D, LOW);
    Play();
  }
  
  // B椅子 放 D位置 觸發 -> 中控機關 -> 判斷暗門開啟
  if (mfrc522.uid.uidByte[0] == RFID_B[0] && mfrc522.uid.uidByte[1] == RFID_B[1] && mfrc522.uid.uidByte[2] == RFID_B[2] && mfrc522.uid.uidByte[3] == RFID_B[3])
  {
    digitalWrite(secretDoor, HIGH); // 發送暗門開啟訊號
  }

  // D椅子 放 D位置 觸發 -> 中控機關 -> 判斷教職員門開啟
