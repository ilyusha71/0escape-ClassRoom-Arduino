...
const int maraca[4] = {225,162,52,0};
const int doll[4] = {155,90,51,0};
/* Output */
const int passMaraca = 2;
const int passDoll = 3;
...
void setup() {
    ...
    pinMode(passDoll,OUTPUT);
    digitalWrite(passMaraca, LOW);
    digitalWrite(passDoll, HIGH);
    Serial.println("Ghost House - Cabinet04 2016/07/24 iLYuSha Wakaka KocmocA");
}
...
void loop()
{ 
  if(mfrc522.uid.uidByte[0] == maraca[0] && mfrc522.uid.uidByte[1] == maraca[1] && mfrc522.uid.uidByte[2] == maraca[2] && mfrc522.uid.uidByte[3] == maraca[3])
    digitalWrite(passMaraca, HIGH);
  else if(mfrc522.uid.uidByte[0] == doll[0] && mfrc522.uid.uidByte[1] == doll[1] && mfrc522.uid.uidByte[2] == doll[2] && mfrc522.uid.uidByte[3] == doll[3])
    digitalWrite(passDoll, LOW);
