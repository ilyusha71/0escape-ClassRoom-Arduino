...
const int poker[4] = {245,184,214,101};
const int kendama[4] = {163,58,48,0};
const int rag[4] = {149,33,212,101};
/* Output */
...
void setup() {
    ...
    Serial.println("Ghost House - Cabinet03 2016/07/24 iLYuSha Wakaka KocmocA");
}
...
void loop()
{ 
  if(mfrc522.uid.uidByte[0] == poker[0] && mfrc522.uid.uidByte[1] == poker[1] && mfrc522.uid.uidByte[2] == poker[2] && mfrc522.uid.uidByte[3] == poker[3])
    digitalWrite(passPoker, HIGH);
  else if(mfrc522.uid.uidByte[0] == kendama[0] && mfrc522.uid.uidByte[1] == kendama[1] && mfrc522.uid.uidByte[2] == kendama[2] && mfrc522.uid.uidByte[3] == kendama[3])
    digitalWrite(passKendama, LOW);
  else if(mfrc522.uid.uidByte[0] == rag[0] && mfrc522.uid.uidByte[1] == rag[1] && mfrc522.uid.uidByte[2] == rag[2] && mfrc522.uid.uidByte[3] == rag[3])
